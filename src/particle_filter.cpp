...
if (is_initialized) {
  ...
  num_particles = 20; // Initializing number of particles
...
/* normal distributions */
  ...
  is_initialized = true; // filter initialized
}
...
/* new state*/
  for (int i = 0; i < num_particles; i++) {
    ...
    if ( fabs(yaw_rate) < 0.00001 )  // yaw constant
	{ 
...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
  ...
  for (unsigned int i = 0; i < nObservations; i++) 
    ...
    observations[i].id = mapId; // Updating observation identifier.
  }
}
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for (int i = 0; i < num_particles; i++) 
    ...
    dataAssociation(inRangeLandmarks, mappedObservations); // Observations association to the landmark
...
void ParticleFilter::resample() {
  ...
  uniform_real_distribution<double> distDouble(0.0, maxWeight); // Creating distribution
  uniform_int_distribution<int> distInt(0, num_particles - 1);
  int index = distInt(gen); // index generating
  double beta = 0.0;
